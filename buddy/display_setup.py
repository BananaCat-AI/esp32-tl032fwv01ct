...
def init_display():
    """Initialize the TL032FWV01CT display and return the FramebufferDisplay."""
    print("[display] Sending init sequence...")
    board.I2C().deinit()
    ...
    print("[display] Creating framebuffer...")
    fb = dotclockframebuffer.DotClockFramebuffer(
        **dict(board.TFT_PINS),
        width=PHYS_WIDTH, height=PHYS_HEIGHT,
        **TFT_TIMINGS,
    )
    display = FramebufferDisplay(fb, auto_refresh=False, rotation=ROTATION)
    print(f"[display] Ready: {display.width}x{display.height}, mem={gc.mem_free()}")
    return display
